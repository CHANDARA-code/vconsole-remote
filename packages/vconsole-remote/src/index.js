...
const BINARY_CONTENT_TYPE =
  /^(image|video|audio|font)\/|^application\/(octet-stream|pdf|zip|gzip|x-gzip|wasm|x-protobuf|vnd\.android\.package-archive)/i;
...
class VConsoleRemote {
  ...
  _normalizeSecurityOptions(provided) {
    ...
    return {
      ...
      sensitiveHeaders: toLowerList(
        input.sensitiveHeaders,
        DEFAULT_SENSITIVE_HEADERS,
      ),
      sensitiveBodyKeys: toLowerList(
        input.sensitiveBodyKeys,
  ...
  _describeRequestBody(body) {
    ...
    try {
      ...
      if (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView(body)) {
        const name =
          (body.constructor && body.constructor.name) || "TypedArray";
    ...
    try {
      ...
      for (const [name, value] of entries) {
        if (
          ...
          lines.push(
            `${name}: [file ${filename}, ${value.type || "unknown type"}, ${this._formatBytes(value.size)}]`,
          );
        } else {
          const masked =
            this.security.maskSensitiveData && this._isSensitiveKey(name)
              ? REDACTED_VALUE
              : String(value);
  ...
  _summarizeResponseBody(headers) {
    ...
    if (contentType && BINARY_CONTENT_TYPE.test(contentType)) {
      const size = Number.isFinite(declaredLength)
        ? `, ${this._formatBytes(declaredLength)}`
        : "";
    ...
    if (
      ...
      declaredLength > this.maxBodyBytes
    ) {
  ...
  _isSensitiveHeader(name) {
    return (
      this.security.sensitiveHeaders.indexOf(String(name).toLowerCase()) !== -1
    );
  }

  _isSensitiveKey(name) {
    return (
      this.security.sensitiveBodyKeys.indexOf(String(name).toLowerCase()) !== -1
    );
  }
  ...
  _maskHeaders(headers) {
    if (
      !this.security.maskSensitiveData ||
      !headers ||
    ...
    for (const key of Object.keys(headers)) {
      ...
      const schemeMatch =
        typeof value === "string" &&
        value.match(/^(Bearer|Basic|Digest|Token)\s+/i);
      masked[key] = schemeMatch
        ? `${schemeMatch[1]} ${REDACTED_HEADER}`
        : REDACTED_HEADER;
    }
    return masked;
  }
  ...
  _maskBody(body) {
    ...
    // Fast-path: Check if any sensitive key is present in the text before doing expensive work.
    // If none of the sensitive keys appear anywhere in the body string, skip parsing/masking entirely.
    const lower = body.toLowerCase();
    const hasAnySensitiveKey = this.security.sensitiveBodyKeys.some((key) =>
      lower.includes(key),
    );
    if (!hasAnySensitiveKey) {
      return body;
    }
    ...
    // Safe linear form-encoded check (avoids catastrophic regex backtracking on non-matching strings)
    if (
      body.length < 65536 &&
      body.includes("=") &&
      !body.includes("\n") &&
      ...
      const isForm = pairs.length > 0 && pairs.every((p) => p.includes("="));
      if (isForm) {
        return pairs
          .map((pair) => {
            ...
            return this._isSensitiveKey(decodedKey)
              ? `${key}=${REDACTED_VALUE}`
              : pair;
          })
          .join("&");
  ...
  _maskUrl(url) {
    ...
    const maskedQuery = query
      ...
      .map((pair) => {
        ...
        return this._isSensitiveKey(decodedKey) ||
          this._isSensitiveHeader(decodedKey)
          ? `${key}=${REDACTED_VALUE}`
          : pair;
      })
      .join("&");
  ...
  _parseServerUrls(input) {
    ...
    const isSecure =
      /^https:\/\//i.test(serverStr) || /^wss:\/\//i.test(serverStr);
    const httpProto = isSecure ? "https:" : "http:";
    const wsProto = isSecure ? "wss:" : "ws:";

    const hostAndPath = serverStr
      .replace(/^https?:\/\//i, "")
      .replace(/^wss?:\/\//i, "");
  ...
  _patchConsole() {
    ...
    for (const method of methods) {
      if (typeof window.console[method] === "function") {
        this._originalConsole[method] = window.console[method].bind(
          window.console,
        );
  ...
  _patchFetch() {
    ...
    window.fetch = async (...args) => {
      ...
      const requestId =
        "fetch_" + startTime + "_" + Math.random().toString(36).substr(2, 6);
  ...
  _patchXHR() {
    ...
    XMLHttpRequest.prototype.open = function (method, url, ...rest) {
      this._vr_id =
        "xhr_" + Date.now() + "_" + Math.random().toString(36).substr(2, 6);
  ...
  _initVConsolePlugin() {
    ...
    const BasePluginClass =
      ...
      (typeof window !== "undefined" &&
        window.VConsole &&
        window.VConsole.VConsolePlugin) ||
      class {
        constructor(id, name) {
          this.id = id;
          this.name = name;
    ...
    class RemotePlugin extends BasePluginClass {
      constructor(id, name) {
        ...
        this.on("show", () => {
          if (
            self.options.autoConnect &&
            (!self.ws || self.ws.readyState === WebSocket.CLOSED)
          ) {
            self.connect();
          }
        });
  ...
  _createTabContent() {
    ...
    pinElement.textContent = this.roomPin
      ? `PIN: ${this._formatPin(this.roomPin)}`
      : "PIN: ------";
  ...
  _renderQrCode(canvasElement, pin) {
    const canvas =
      ...
      (typeof document !== "undefined" &&
        document.getElementById("vconsole-remote-qr"));
    ...
    QRCode.toCanvas(canvas, targetUrl, { width: 180, margin: 2 }, (error) => {
      if (error && this._originalConsole && this._originalConsole.error) {
        this._originalConsole.error(
          "[vConsole-Remote] QR Code render error:",
          error,
        );
      }
    });
  ...
  _showAuthPrompt(data) {
    ...
    rejectButton.addEventListener("click", () =>
      this._respondToAuth(authId, false),
    );
    ...
    allowButton.addEventListener("click", () =>
      this._respondToAuth(authId, true),
    );
  ...
  _respondToAuth(authId, approved) {
    ...
    this.updateConnectionStatus(
      approved ? "Developer Connected 🟢" : "Connection Rejected 🟥",
    );
  }
  ...
  _pairingUrl(pin) {
    ...
    return this.roomKey
      ? `${base}?key=${encodeURIComponent(this.roomKey)}`
      : base;
  }
  ...
  getConnectionColor(status) {
    ...
    if (status.includes("Waiting") || status.includes("🟡"))
  ...
  _handlePullStorage(data) {
    ...
    const storagePayload = {
      ...
      storage: {
        localStorage: Object.keys(lsMap).map((k) => ({
          key: k,
          value: lsMap[k],
        })),
        sessionStorage: Object.keys(ssMap).map((k) => ({
          key: k,
          value: ssMap[k],
        })),
        cookies: cookiesStr
          ? cookiesStr
              ...
              .map((c) => {
                const idx = c.indexOf("=");
                return idx > -1
                  ? { key: c.slice(0, idx), value: c.slice(idx + 1) }
                  : { key: c, value: "" };
              })
          : [],
      },
      ...
      if (childNodes) {
        for (let i = 0; i < childNodes.length; i++) {
          ...
          if (child.nodeType === 1) {
            ...
            if (text) {
              item.textContent =
                (item.textContent ? item.textContent + " " : "") + text;
            }
          }
        }
      }
  ...
  _handleExecJS(data) {
    ...
    if (!this.security.allowRemoteEval) {
      const refusal =
        "Remote evaluation is disabled on this device (security.allowRemoteEval is false)";
      this._send({
        type: "exec_result",
        error: refusal,
        output: refusal,
  ...
  destroy() {
    ...
    if (typeof XMLHttpRequest !== "undefined") {
      ...
      if (this._originalXHRSetRequestHeader)
        XMLHttpRequest.prototype.setRequestHeader =
          this._originalXHRSetRequestHeader;
    }
  }
}
